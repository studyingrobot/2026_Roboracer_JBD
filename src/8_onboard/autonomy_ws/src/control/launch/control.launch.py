...
def _launch_setup(context):
    ...
    # minjae changes
    # 다른 제어기는 mpc_profile:=speed_<m/s> 규약을 쓰지만 minjae_pp 는
    # speed:=<m/s> 하나만 받는다. yaml 의 target_speed 를 항상 덮어써서
    # 실차에서 speed 가 조용히 무시되는 일이 없게 한다.
    ...
    if controller == 'minjae_pp':
        ...
        if (not math.isfinite(requested_speed)
            ...
            raise RuntimeError(
                'minjae_pp requires speed:=<m/s> greater than 0 and at most '
                f'{maximum_speed:g}; got {requested_speed!r}.')
        return [
            LogInfo(msg=(
                f'Controller=minjae_pp speed={requested_speed:.2f}m/s')),
            Node(
                package='control',
                ...
                output='screen',
                parameters=[
                    LaunchConfiguration('minjae_params_file').perform(context),
                    {
    ...
    if controller not in ('mpc', 'mpcc'):
        raise RuntimeError(
            ...
            'pure_pursuit, kyeongho_pp, unicorn_l1, forza_map, mpc, '
            'or mpcc.')  # minjae changes
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            default_value=os.path.join(
                package_share, 'config', 'minjae_pp_params.yaml'),
        ),
        DeclareLaunchArgument(
            'speed',
            default_value='1.0',
        ...
        DeclareLaunchArgument(
            ...
            description=(  # minjae changes
                'none, minjae_pp, pure_pursuit, kyeongho_pp, unicorn_l1, '
