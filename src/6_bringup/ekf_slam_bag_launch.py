"""bag 재생 기반 EKF + slam_toolbox + RViz 브링업.
  ...
  ros2 launch src/6_bringup/ekf_slam_bag_launch.py bag:=test_bags/0808_test_3 play:=true

config/local_ekf.yaml 은 수정하지 않는다. VESC 원본 IMU 와의 차이(deg/s, g,
...
EKF_YAML = os.path.join(ROOT, 'config', 'local_ekf.yaml')
SLAM_YAML = os.path.join(ROOT, 'src', '1_slam_mapping', 'config', 'mapper_params_offline.yaml')
IMU_RELAY = os.path.join(ROOT, 'src', '3_localization', 'imu_relay.py')
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument('bag', default_value='test_bags/0808_test_2',
                              description='재생할 bag 경로'),
